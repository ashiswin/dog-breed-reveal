...
async function getRedis() {
  if (typeof process.env.UPSTASH_REDIS_REST_URL === "undefined") return null;
  if (!process.env.UPSTASH_REDIS_REST_URL || !process.env.UPSTASH_REDIS_REST_TOKEN) {
    console.log("[store] Redis env vars not set, using memory");
    return null;
  }
  ...
  try {
    ...
    console.error("[store] Redis connection failed:", e.message);
    redis = null;
  }
  return redis;
}
...
async function getStore(key) {
  ...
  if (client) {
    try {
      ...
      console.error(`[store] Redis get failed for ${key}:`, e.message);
    }
  }
  return memStore[key];
}
...
async function setStore(key, value) {
  ...
  if (client) {
    try {
      ...
      console.log(`[store] Saved ${key} to Redis`);
    } catch (e) {
      console.error(`[store] Redis set failed for ${key}:`, e.message);
    }
  }
}
...
export async function setAnswers(dog1, dog2, image1, image2) {
  const answers = { dog1, dog2, image1: image1 || null, image2: image2 || null };
